...
def run(args):
    ...
    def count_parameters(model):
        ...
        for p in model.parameters():
            if p.requires_grad: answer += p.numel()
        return answer
    ...
    model.to(device)
    if args.is_tune: results = atio.do_test(model, dataloader['test'], mode="TEST")
    else: results = atio.do_test(model, dataloader['test'], mode="TEST")
    del model
...
def run_normal(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        if i == 0 and args.data_missing: missing_rate = str(args.missing_rate[0])
        setup_seed(seed)
        args.seed = seed
    ...
    if not os.path.exists(args.res_save_dir): os.makedirs(args.res_save_dir)
    if os.path.exists(save_path): df = pd.read_csv(save_path)
    else: df = pd.DataFrame(columns=["Model"] + criterions)
    res = [args.modelName]
    for c in criterions:
...
def set_log(args):
    ...
    for ph in logger.handlers: logger.removeHandler(ph)
...
def parse_args():
    ...
    parser.add_argument('--modelName', type=str, default='MIT-FRNet', help='support tfn/mult/misa/tfr_net/MIT-FRNet')
    parser.add_argument('--datasetName', type=str, default='mosi', help='support mosi/sims')
