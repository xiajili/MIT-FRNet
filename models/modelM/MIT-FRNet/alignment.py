...
class CM_ATTN(nn.Module):
    ...
    def get_network(self, self_type='l', layers=-1):
        if self_type in ['l', 'al', 'vl']: embed_dim, attn_dropout, num_heads, position_embedding = self.d_l, self.attn_dropout, self.num_heads, True
        elif self_type in ['a', 'la', 'va']: embed_dim, attn_dropout, num_heads, position_embedding = self.d_a, self.attn_dropout_a, self.num_heads, True
        elif self_type in ['v', 'lv', 'av']: embed_dim, attn_dropout, num_heads, position_embedding = self.d_v, self.attn_dropout_v, self.num_heads, True
        elif self_type == 'l_mem': embed_dim, attn_dropout, num_heads, position_embedding = self.d_l, self.attn_dropout, self.num_heads, True
        elif self_type == 'a_mem': embed_dim, attn_dropout, num_heads, position_embedding = self.d_a, self.attn_dropout, self.num_heads, True
        elif self_type == 'v_mem': embed_dim, attn_dropout, num_heads, position_embedding = self.d_v, self.attn_dropout, self.num_heads, True
        elif self_type == 'l_final': embed_dim, attn_dropout, num_heads, position_embedding = self.seq_lens[0], self.attn_dropout, 25, False 
        elif self_type == 'a_final': embed_dim, attn_dropout, num_heads, position_embedding = self.seq_lens[1], self.attn_dropout, 25, False
        elif self_type == 'v_final': embed_dim, attn_dropout, num_heads, position_embedding = self.seq_lens[2], self.attn_dropout, 25, False  
        else: raise ValueError("Unknown network type")
        return TransformerEncoder(embed_dim=embed_dim, num_heads=num_heads, layers=max(self.layers, layers), attn_dropout=attn_dropout, relu_dropout=self.relu_dropout, res_dropout=self.res_dropout, embed_dropout=self.embed_dropout, attn_mask=self.attn_mask, position_embedding=position_embedding)
