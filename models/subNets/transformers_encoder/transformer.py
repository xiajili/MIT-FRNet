...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0, embed_dropout=0.0, attn_mask=False, position_embedding=False):
        super().__init__()
    ...
    def forward(self, x_in, x_in_k = None, x_in_v = None):
        ...
        if self.embed_positions is not None: x += self.embed_positions(x_in.transpose(0, 1)[:, :, 0]).transpose(0, 1) 
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None: x = layer(x, x_k, x_v)
            else: x = layer(x)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        if x_k is None and x_v is None: x, _ = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
        else:
...
def buffered_future_mask(tensor, tensor2=None):
    ...
    if tensor.is_cuda: future_mask = future_mask.to(tensor.device)
