...
class SimilarityLossAVT(torch.nn.Module):
    def __init__(self, result_loss, loss_similarity, gamma=0.5):
        ...
        if loss_similarity == 'KL': self.similarity_loss = F.kl_div
        elif loss_similarity == 'Cosine': self.similarity_loss = nn.CosineSimilarity(dim=-1)
...
class SMLossAVT(torch.nn.Module):
    def __init__(self, sm_loss, sm_level):
        ...
        if sm_loss.lower() == 'kl': self.loss_func = MultVariateKLD(reduction='mean')
        elif sm_loss.lower() == 'gau': self.loss_func = nn.GaussianNLLLoss(reduction='mean', full=False)
        else: raise NotImplementedError
    def forward(self, inputs):
        ...
        if self.sm_loss == 'kl':
            ...
            if self.sm_level == 'batch':
                ...
                loss_all = (output_a + output_v + output_t)/3
            elif self.sm_level == 'vec':   #defualt
                var = sumed.var(dim=-1)
...
class UnivariateKLD(torch.nn.Module):
    ...
    def forward(self, mu1, mu2, var_1, var_2): 
        ...
        if self.reduction == 'mean': kl_agg = torch.mean(kl)
        elif self.reduction == 'sum': kl_agg = torch.sum(kl)
        else: raise NotImplementedError(f'Reduction type not implemented: {self.reduction}')
        return kl_agg
class MultVariateKLD(torch.nn.Module):
    ...
    def forward(self, mu1, mu2, var_1, var_2): 
        ...
        term_3 = torch.matmul(torch.matmul((mu2 - mu1).unsqueeze(-1).transpose(2, 1), sigma_diag_2_inv), (mu2 - mu1).unsqueeze(-1)).flatten()
        n = mu1.shape[1]
...
def make_weights_for_balanced_classes(labels, nclasses=7):
    ...
    N = float(sum(count)) 
    for i in range(nclasses): weight_per_class[i] = N / float(count[i]+float("1e-8"))
    weight = [0] * len(labels)
...
def make_weights_for_balanced_classes_from_list(labels, nclasses=7):
    count = [0] * nclasses
    for item in labels: count[int(item)] += 1
    weight_per_class = [0.] * nclasses
    N = float(sum(count))  
    for i in range(nclasses): weight_per_class[i] = N / float(count[i])
    weight = [0] * len(labels)
    for idx, val in enumerate(labels): weight[idx] = weight_per_class[int(val)]
    return weight
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=120000):
        ...
        else: pe[:, 1::2] = torch.cos(position * div_term)[:, :-1]
